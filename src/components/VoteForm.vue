...
<template>
  <form @submit.prevent="handleSubmit" class="flex flex-col items-center gap-3 w-80 md:w-96 mx-auto bg-neutral-100 p-4 rounded-2xl">
    <div class="w-full flex items-center gap-1">
      ...
      <button type="submit" class="cursor-pointer bg-blue-600 hover:bg-blue-700 py-1 px-4 rounded-sm w-[20%] text-white">Add</button>
    </div>
    <span class="w-full text-red-700 text-sm text-center">{{error}}</span>
    <button type="button" @click="toggleVoteForm" class="w-full cursor-pointer bg-blue-600 hover:bg-blue-700 py-1 px-4 rounded-sm text-white">Close</button>
  </form>
</template>